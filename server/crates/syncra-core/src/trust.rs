...
pub fn fingerprint_words(parts: &[&[u8]]) -> Vec<String> {
    ...
    digest
        ...
        .map(|pair| {
            ...
            // `%` здесь можно, в отличие от `generator::random_index`, где от
            // него отказались. Разница в том, что там перекос достаётся
            // паролю — секрету, у которого энтропия обязана быть ровно такой,
            // как обещано, — а здесь он достаётся ОТПЕЧАТКУ, который человек
            // сверяет глазами. При 1073 словах индексы 0–82 выпадают на 1.6 %
            // чаще прочих; для сорока бит отпечатка это не меняет ничего, а
            // отбраковка значений сделала бы слова недетерминированной
            // функцией ключей — то есть сломала бы саму сверку.
