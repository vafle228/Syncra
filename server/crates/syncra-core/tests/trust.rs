...
/// Второе устройство в `devices`. Сопряжения ещё нет (S2), поэтому «сопряжённое»
/// устройство тест кладёт в файл сам — тем же приёмом, что `at_rest.rs` смотрит
/// в хранилище мимо ядра. Публичного API писать под тест не стали: он появится
/// вместе с `confirm_pairing`, а не раньше.
