...
//!
//! Здесь проверяется ядро БЕЗ сети: «второе устройство» — второе ядро в том же
//! процессе, на своём хранилище и со своим мастер-паролем, а обмен кодами делает
//! то, что в жизни делает камера — строка пейлоада из одного ядра подаётся в
//! другое. Так сопряжение и работает по QR: сеть ему для этого не нужна.
//!
//! Всё, чему сеть нужна, — поиск устройства по шести символам и рассказ об
//! успехе показавшей стороне — живёт в `net::Node` и проверяется `tests/net.rs`.
//!
...
#[test]
fn a_hand_typed_code_needs_the_network() {
    // Шесть символов не несут публичного ключа: это имя сеанса, по которому
    // устройство ИЩУТ в локальной сети (§2.2). Поиск появился в S3, но живёт он
    // в `net::Node` — снаружи мьютекса ядра, потому что ожидание в сети под этим
    // замком заморозило бы весь UI.
    //
    // Ядро само по себе поэтому по-прежнему отвечает «рядом такого нет»: у него
    // нет и не должно быть способа кому-то позвонить. Ответ тот же, что и до
    // S3, — изменилось только то, КТО его теперь может не дать. Сам поиск
    // проверяется в `tests/net.rs`.
    let mut core = unlocked();
...
fn nothing_travels_yet() {
    ...
    // ГРАНИЦА ШАГА: переносить записи нечем, пока нет манифеста и диффа (S4).
    // Ноль — правда о состоянии, а не заглушка.
