...
>
> RESULT (ручной проход по мок-ядру): пройден целиком в браузере против `npm run dev`, в обеих
> темах и на двух акцентах. Совпало с макетом всё, кроме одного — см. дефект ниже.
> Проверено по маршруту из чек-листа: список (поиск занимает колонку, «Старые пароли» реально
> переупорядочивают в Google → GitHub → Steam), карточка (кнопки шапки вровень, блока «Версия»
> нет, TOTP тикает `884 257` → `25 с` → `19 с` с кольцом, заметки — две скелетон-полосы, подвал
> считает «хранится на 3 устройствах»), «···» (углы скруглены), диалог удаления (порядок,
> подсказка не вылезает, янтарная полоса внутри красной карточки), конфликт (полосатая оболочка,
> различия в чипах, совпадения приглушены), форма (секция с точками и «Управление секциями…»,
> заметки без соседней кнопки, TOTP в состоянии «Ключ добавлен»), секции (шапка сливается с
> панелью, свотчи в ряд, выбор не сдвигает соседей), устройства (язык присутствия, «Отозвать»
> краснеет только под курсором — проверено наведением), настройки (четыре вкладки, «Данные» и
> компактные диалоги CSV/бэкапа без «Показать»), сопряжение (узкий мастер, пара кнопок в подвале).
> Ошибок в консоли нет.
>
> ДЕФЕКТ, найденный этим проходом (5.1): свотчи акцента показывали не свои палитры, а текущий
> акцент документа — все четыре одного цвета. Причина: палитры были описаны как
> `:root[data-accent=…]`, а такой селектор совпадает только с `<html>`, поэтому `data-accent` на
> самом свотче ничего не включал; у мяты атрибута не было вовсе. Починено: правила палитр больше
> не привязаны к `:root`, добавлен блок `[data-accent='mint']` (через именованные
> `--sy-accent-mint*`, без дублирования значений) и светлые варианты получили вторым селектором
> потомка `[data-theme='light'] [data-accent=…]`. Проверено `getComputedStyle` в обеих темах:
> четыре свотча дают четыре разных цвета. Тест в `SettingsView.spec.ts` смотрел только на один
> свотч (янтарь) и потому дефект пропускал — теперь проверяет все четыре.
>
